...
def show():
    """Admin view to see any student's transcript for any semester"""
    render_page_title("📃", "Student Transcript Viewer")
    
    # Connect to database
    conn = get_db_connection()
    
    # Get all students
    students = conn.execute("SELECT id, student_id, name, dept FROM students ORDER BY name").fetchall()
    ...
    # Convert students to a format for selectbox
    student_options = {f"{s['student_id']} - {s['name']} ({s['dept']})": s['id'] for s in students}
    selected_student_name = st.selectbox("Choose a student:", options=list(student_options.keys()))
    selected_student_id = student_options[selected_student_name]
    
    # Get student details
    student = conn.execute("SELECT * FROM students WHERE id = ?", (selected_student_id,)).fetchone()
    ...
    if not academic_sessions:
        st.warning("No academic sessions found. Please create sessions first.")
        conn.close()
        return
    ...
    with col2:
        # Get department from student info
    ...
    semester_options = [s['semester'] for s in enrolled_semesters]
    selected_semester = st.selectbox("Select Semester to View:", options=semester_options)
    
    # Show transcript for selected semester
    st.markdown(f"## {selected_semester}")
    ...
    courses_with_grades = conn.execute("""
        ...
        ORDER BY c.code
    """, (selected_student_id, selected_semester, selected_student_id, selected_semester, selected_student_id, 
          selected_student_id, selected_semester, selected_semester, selected_student_id, selected_semester)).fetchall()
    
    if not courses_with_grades:
        st.info(f"No courses found for {selected_semester}.")
        conn.close()
        return
    ...
    for course in courses_with_grades:
        ...
        if course['mid'] is not None or course['final'] is not None:
            grades_data.append({
                ...
                "Attendance": f"{attendance_grade:.1f}",
                "Assignments": f"{assign_grade:.1f}",
                "Class Tests": f"{test_grade:.1f}",
                "Midterm": midterm,
                "Final": final,
                "Total": f"{total_grade:.1f}",
                "Grade": letter_grade,
    ...
    # View all semesters option
    if st.checkbox("View All Semesters"):
        st.subheader("All Semesters GPA Summary")
        ...
        for semester in enrolled_semesters:
            ...
            # Get semester credits and points
            semester_data = conn.execute("""
                SELECT 
                    SUM(c.credit_hour) as credits,
                    SUM(
                        c.credit_hour * 
                        CASE
                            WHEN (g.mid + g.final) >= 80 THEN 4.0
                ...
                FROM enrollments e
                JOIN courses c ON e.course_id = c.id
                JOIN grades g ON g.student_id = e.student_id AND g.course_id = e.course_id AND g.semester = e.semester
                WHERE e.student_id = ? AND e.semester = ?
            """, (selected_student_id, semester_name)).fetchone()
            
            if semester_data and semester_data['credits']:
                semester_credits = semester_data['credits']
                semester_points = semester_data['points']
